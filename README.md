...
### Сборка фронтенда через деплойный скрипт

Для продакшен-деплоя фронтенд собирается автоматически скриптом `deploy.sh`.  
Скрипт выполняет следующие действия:

1. Устанавливает Node.js-пакеты (`npm install`).
2. Собирает статику и фронтенд-бандлы (`python manage.py collectstatic --noinput`).
3. Перезапускает сервис Gunicorn.
4. Перезагружает Nginx безопасным способом (`nginx -s reload`).

Таким образом, вам не нужно запускать Parcel вручную для продакшен-версии.  
Просто выполните:
